...
static void dump_fdt_header(unsigned char *fdt)
{
	unsigned int i;
	unsigned long long j;

	struct fdt_header hdr = {
		.magic =		CPU_TO_FDT32(((unsigned int *)fdt)[0]),
		.totalsize =		CPU_TO_FDT32(((unsigned int *)fdt)[1]),
		.off_dt_struct =	CPU_TO_FDT32(((unsigned int *)fdt)[2]),
		.off_dt_strings =	CPU_TO_FDT32(((unsigned int *)fdt)[3]),
		.off_mem_rsvmap =	CPU_TO_FDT32(((unsigned int *)fdt)[4]),
		.version =		CPU_TO_FDT32(((unsigned int *)fdt)[5]),
		.last_comp_version =	CPU_TO_FDT32(((unsigned int *)fdt)[6]),
		.boot_cpuid_phys =	CPU_TO_FDT32(((unsigned int *)fdt)[7]),
		.size_dt_strings =	CPU_TO_FDT32(((unsigned int *)fdt)[8]),
		.size_dt_struct =	CPU_TO_FDT32(((unsigned int *)fdt)[9]),
	};
	...
	printf("    totalsize:          0x%08x\n"
	       "    off_dt_struct:      0x%08x\n"
	       "    off_dt_strings:     0x%08x\n",
	       hdr.totalsize, hdr.off_dt_struct, hdr.off_dt_strings);
	printf("    off_mem_rsvmap:     0x%08x\n"
	       "    version:            0x%08x\n"
	       "    last_comp_version:  0x%08x\n",
	       hdr.off_mem_rsvmap, hdr.version, hdr.last_comp_version);
	printf("    boot_cpuid_phys:    0x%08x\n"
	       "    size_dt_strings:    0x%08x\n"
	       "    size_dt_struct:     0x%08x\n",
	       hdr.boot_cpuid_phys, hdr.size_dt_strings, hdr.size_dt_struct);
	printf("\n");
}
...
# error Unknown CPU type
	...
	for (i = 0; i < 16 * 1024; i += 4) {
		if (memcmp((char *)ntohl(hdr->ih_load) + i, "OWRTDTB:", 8) == 0) {
			printf("   Device Tree Blob at: 0x%08x\n", (char *)ntohl(hdr->ih_load) + i);
			i += 8;
			dump_fdt_header((unsigned char *)ntohl(hdr->ih_load) + i);
			break;
		}
	}
