...
static void set_YYTD_ID_in_env(unsigned int val)
{
	char str_buff[128];

	memset(str_buff, 0, sizeof(str_buff));

	sprintf(str_buff, "%x", val);
	setenv("YYTD_ID", str_buff);
}
...
static void gpio_init(void)
{
	...
	char *s;
	unsigned int id;
	char kernel_cmdline_buf[128] = { 0 };
	int i = 0;

	printf("\n\n");

	/*
	 * GPIO2, USB MUXer to USB PORT
	 * GPIO44, WLED ---> GPIO OUT
	 * GPIO43, EPHY P0 LED ---> EPHY P0 LED
	 * GPIO36, PERST ---> Recovery GPIO IN
	 * GPIO37, REF_CLK ---> GPIO_ID0
	 * GPIO38, WDT_RST_N ---> GPIO_ID1
	 * GPIO39, EPHY P4 LED ---> GPIO_ID2
	 * GPIO40, EPHY P3 LED ---> GPIO_ID3
	 * GPIO41, EPHY P2 LED ---> GPIO_ID4
	 */
	val = RALINK_REG(RT2880_SYS_CNTL_BASE + GPIOMODE);
	val &= ~(0x03 << 6);
	val |= ((0x01 << 6) | (0x01 << 14) | (0x01 << 16) | (0x01 << 18));
	RALINK_REG(RT2880_SYS_CNTL_BASE + GPIOMODE) = val;

	val = RALINK_REG(RT2880_SYS_CNTL_BASE + GPIOMODE2);
	val &= ~((0x03 << 0) | (0x03 << 2) | (0x03 << 6) | (0x03 << 8) | (0x03 << 10));
	val |= ((0x01 << 0) | (0x00 << 2) | (0x01 << 6) | (0x01 << 8) | (0x01 << 10));
	RALINK_REG(RT2880_SYS_CNTL_BASE + GPIOMODE2) = val;

	/*
	 * only GPIO2 and GPIO44 dir out, others are dir in
	 * all pins default to dir in
	 */
	val = RALINK_REG(RT2880_REG_PIODIR + PIO_DIR0);
	val |= 1 << 2;
	RALINK_REG(RT2880_REG_PIODIR + PIO_DIR0) = val;
	RALINK_REG(RT2880_REG_PIORESET) = 0x01 << 2;

	val = RALINK_REG(RT2880_REG_PIODIR + PIO_DIR1);
	val |= 1 << 12;
	RALINK_REG(RT2880_REG_PIODIR + PIO_DIR1) = val;
	/* RALINK_REG(RT2880_REG_PIOSET + 0x04) = 0x01 << 12; */

	val = (RALINK_REG(RT2880_REG_PIODATA + 0x04) >> 5) & 0x1f;
	printf("\033[7m*** YYTD ID: %x ***\033[0m\n", val);
	if ((s = getenv("YYTD_ID")) != NULL) {
		id = simple_strtoul(s, NULL, 16);
		if (id == val) {
			/* go as usual */
		} else {
			printf("\033[7m\t*** UPDATE ***\033[0m\n");
			set_YYTD_ID_in_env(val);
			saveenv();
		}
	} else {
		/* first boot, unset some unused envs */
		/* setenv("bootcmd", NULL); */
		set_YYTD_ID_in_env(val);
		saveenv();
		while(i < 666) {
	...
	/* MV88E6071 NO_CPU HIGH */
	RALINK_REG(RT2880_REG_PIOSET) = 0x01 << 14;
	// RALINK_REG(RT2880_REG_PIORESET) = 0x01 << 17;
}
...
__attribute__((nomips16)) void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	if (((RALINK_REG(RT2880_REG_PIODATA + 0x04) >> 5) & 0x10) == 0x10) { /* FS FSU, 4G module power via currennt trasformer */
		if(reg & (RT2880_WDRST | RT2880_SWSYSRST)) {
			/* set GPIO1 as output to trigger 4G module power down */
			printf("___ FORCE 4G POWER DOWN! ___\n");
			u32 val = RALINK_REG(RT2880_REG_PIODIR + PIO_DIR0);
			val |= 1 << 1;
			RALINK_REG(RT2880_REG_PIODIR + PIO_DIR0) = val;
			RALINK_REG(RT2880_REG_PIOSET) = 0x01 << 1;
		}
	}
	if(reg & RT2880_WDRST ){
		printf("***********************\n");
...
/*config bootdelay via environment parameter: bootdelay */
	...
	while (timer1 > 0) {
		...
		for (i=0; i<100; ++i) {
			if ((my_tmp = tstc()) != 0) {	/* we got a key press	*/
				...
				BootType = getc();
				if ((BootType < '0' || BootType > '5') && (BootType != '7') && (BootType != '8') && (BootType != '9') && (BootType != 'b') && (BootType != 'm') && (BootType != 'c') && (BootType != 'x'))
					BootType = '3';
				printf("\n\rYou choosed %c\n\n", BootType);
				break;
			}
	...
	if(BootType == '3') {
		...
		case 'b':
			...
			break;
		case 'x':
			printf("\nLoad squashfs sysupgrade then write to Flash via USB Storage.\n");
			...
			sprintf(addr_str, "0x%X", CFG_LOAD_ADDR);
			argv[3] = &addr_str[0];
			argv[4] = "YYTD-7688-squashfs-sysupgrade.bin";
			setenv("autostart", "no");

			if (do_fat_fsload(cmdtp, 0, argc, argv)) {
				printf("Could not find YYTD-7688-squashfs-sysupgrade.bin\n");
			} else {
				NetBootFileXferSize=simple_strtoul(getenv("filesize"), NULL, 16);
				printf("writing YYTD-7688-squashfs-sysupgrade.bin to flash\n");
				raspi_erase_write((char *)CFG_LOAD_ADDR, CFG_KERN_ADDR - CFG_FLASH_BASE, NetBootFileXferSize);
			}
			led_on();
			while(1) {
...
FINAL:
	printf("\n\n");
	for (j = 0; j < 2; j++)
		cal_debug("[%02X%02X%02X%02X]\n",min_coarse_dqs[j],min_fine_dqs[j], max_coarse_dqs[j],max_fine_dqs[j]);
	cal_debug("DDR Calibration DQS reg = %08X\n",reg);
